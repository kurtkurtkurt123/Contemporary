...
    };
    
    const roleName = user?.Role || user?.role || user?.user_role;
    const roleDisplay = roleName?.toLowerCase() === 'uo_staff'
        ? 'Unofficial Staff'
        : roleName || 'Guest';

    // Safely get name initials for the avatar
    const userInitials = (user?.FirstName || user?.user_fn || 'U').charAt(0) + (user?.LastName || user?.user_ln || 'L').charAt(0).toUpperCase();
...
                <span className="text-gray-800 font-medium hidden md:inline">{user?.FirstName || user?.user_fn}</span> 
                
                {isOpen ? (
...
                        <p className="text-sm font-semibold text-gray-900 truncate mb-2">{(user?.FirstName || user?.user_fn) + ', ' + (user?.LastName || user?.user_ln) || 'User'}</p>
                        <p className="text-xs text-gray-500 capitalize">{roleDisplay}</p>
                        <p className='text-xs text-gray-300 truncate'>{user?.UserCode || user?.user_code}</p>
                    </div>
...
        const baseLinks = [];
        
        // 1. Home link is mandatory for all logged-in users (usually the landing page/hub)
        baseLinks.push({ to: '/home', label: 'Home', icon: HomeIcon }); 

        // 2. STUDENT LINKS
        if (['student'].includes(role)) {
            // Added Dashboard link to the new dedicated page wrapper
            baseLinks.push({ to: '/dashboard', label: 'Dashboard', icon: HomeIcon }); 
            baseLinks.push({ to: '/lessons', label: 'Lessons', icon: BookOpenIcon }); 
...
        // 4. STAFF/INSTRUCTOR LINKS (Staff handles submissions/grading)
        if (role === 'staff' || role === 'uo_staff' || role === 'admin') {
            baseLinks.push({ to: '/tasks', label: 'Check Activity', icon: CheckCircleIcon });
        }
        
        if (role === 'admin') {
            baseLinks.push({ to: '/students', label: 'Students', icon: UsersIcon });
